...
# wait if any of robots' global costmap map is not received yet
    rospy.loginfo('Waiting for global costmaps')
    for i in range(0, n_robots):
        while (len(globalmaps[i].data) < 1):
            rospy.sleep(0.1)
            pass
    rospy.loginfo("Global costmaps are received!")
    ...
    if len(namespace) > 0:
        for i in range(0, n_robots):
            rospy.loginfo('Waiting for transform ' + global_frame[1:] + ' to ' + namespace+str(
                i+namespace_init_count)+'/'+robot_frame);
            tfLisn.waitForTransform(global_frame[1:], namespace+str(
                i+namespace_init_count)+'/'+robot_frame, rospy.Time(0), rospy.Duration(10.0))
            rospy.loginfo('Transform ' + global_frame[1:] + ' to ' + namespace+str(
                i+namespace_init_count)+'/'+robot_frame + ' received!')
    elif len(namespace) == 0:
        rospy.loginfo('Waiting for transform ' + global_frame[1:] +  ' to /'+robot_frame)
        tfLisn.waitForTransform(
            global_frame[1:], '/'+robot_frame, rospy.Time(0), rospy.Duration(10.0))
        rospy.loginfo('Transform ' + global_frame[1:] + ' to /'+robot_frame + ' received!')
