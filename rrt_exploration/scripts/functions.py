    ...
    def __init__(self, name):
        ...
        while cond == 0:
            try:
                ...
                rospy.loginfo('Robot transform received!')
                cond = 1
        ...
        rospy.loginfo('Waiting for "' + self.name + '/move_base' + '" action server')
        self.client.wait_for_server()
        rospy.loginfo(self.name + '/move_base' + '" action server ready!')
        robot.goal.target_pose.header.frame_id = self.global_frame
        robot.goal.target_pose.header.stamp = rospy.Time.now()

        rospy.loginfo('Waiting for "' + self.name+self.plan_service + '" service')
        rospy.wait_for_service(self.name+self.plan_service)
        rospy.loginfo(self.name+self.plan_service + '" service ready!')
