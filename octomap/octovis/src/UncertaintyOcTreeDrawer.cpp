...
#include <octovis/UncertaintyOcTreeDrawer.h>

namespace octomap {

  UncertaintyOcTreeDrawer::UncertaintyOcTreeDrawer()
    : OcTreeDrawer() {
  }
  ...
  void UncertaintyOcTreeDrawer::setOcTree(const AbstractOcTree& tree_pnt,
                                    const octomap::pose6d& origin_,
                                    int map_id_) {

    const UncertaintyOcTree& tree = ((const UncertaintyOcTree&) tree_pnt);
    ...
    for(UncertaintyOcTree::tree_iterator it = tree.begin_tree(this->m_max_tree_depth),
    ...
    // setup GL arrays for cube quads and cube uncertainties
    ...
    unsigned int uncertainty_idx_occupied(0), uncertainty_idx_occupied_thres(0);
      ...
      if (it.isLeaf()) { // voxels for leaf nodes
        ...
        if (tree.isNodeOccupied(*it)){ // occupied voxels
          if (tree.isNodeAtThreshold(*it)) {
            ...
            uncertainty_idx_occupied_thres =  setCubeColorUncertainty(it->getUncertainty(),
                                                         uncertainty_idx_occupied_thres, &m_occupiedThresColorArray);
          }
          else {
            idx_occupied = generateCube(voxel, cube_template, idx_occupied, &m_occupiedArray);
            uncertainty_idx_occupied = setCubeColorUncertainty(it->getUncertainty(),
                                                  uncertainty_idx_occupied, &m_occupiedColorArray);
          }
        }
        else if (showAll) { // freespace voxels
          if (tree.isNodeAtThreshold(*it)) {
