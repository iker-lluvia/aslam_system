...
octomap::UncertaintyOcTree uncertaintyTreeTmp(0);
...
void ViewerGui::addOctree(octomap::AbstractOcTree* tree, int id, octomap::pose6d origin) {
  // is id in use?
      ...
      if (foundRecord && r->octree->getTreeType().compare(tree->getTreeType()) !=0){
        ...
        else if (dynamic_cast<UncertaintyOcTree*>(tree)) {
          r->octree_drawer = new UncertaintyOcTreeDrawer();
        } else{
        ...
        if (dynamic_cast<OcTree*>(tree)) {
          ...
          otr.octree_drawer = new UncertaintyOcTreeDrawer();
        }
        else{
          OCTOMAP_ERROR("Could not create drawer for tree type %s\n", tree->getTreeType().c_str());
        }
...
void ViewerGui::openOcTree(){
  ...
  if (tree){
    ...
    if (tree->getTreeType() == "UncertaintyOcTree"){
      // map uncertainty and height map share the same color array and QAction
      ui.actionHeight_map->setText ("Map uncertainty");  // rename QAction in Menu
      this->on_actionHeight_map_toggled(true); // enable uncertainty view
      ui.actionHeight_map->setChecked(true);
    }
  }
  else {
    ...
    for (std::map<int, OcTreeRecord>::iterator it = m_octrees.begin();
      ...
      if (dynamic_cast<OcTree*>(t)) {
        ...
        ((ColorOcTree*) t)->toMaxLikelihood();
          ...
          ((UncertaintyOcTree*) t)->toMaxLikelihood();
        }
    }
    showInfo("Done.", true);
    showOcTree();
