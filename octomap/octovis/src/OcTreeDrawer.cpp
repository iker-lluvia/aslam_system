...
namespace octomap {
  ...
  unsigned int OcTreeDrawer::setCubeColorUncertainty(const float u,
                                                     const unsigned int& current_array_idx,
                                                     GLfloat** glColorArray) {
    ...
    unsigned int colorIdx = current_array_idx;
    // color for all 4 vertices (=one quad)
    for (int k = 0; k < 4; ++k) {
      (*glColorArray)[colorIdx    ] = (double) u;
      (*glColorArray)[colorIdx + 1] = (double) u;
      (*glColorArray)[colorIdx + 2] = (double) u;
      // set Alpha value:
      (*glColorArray)[colorIdx + 3] = 1;
      colorIdx += 4;
    }
    return colorIdx;
  }
