...
int main(int argc, char** argv) {
  ...
  // Test for tree headers and IO factory registry (uncertainty)
  {
    std::cout << "Testing UncertaintyOcTree...\n";

    double res = 0.02;
    std::string filenameUncertainty = "test_io_uncertainty_file.ot";
    UncertaintyOcTree uncertaintyTree(res);
    EXPECT_EQ(uncertaintyTree.getTreeType(), "UncertaintyOcTree");
    UncertaintyOcTreeNode* uncertaintyNode = uncertaintyTree.updateNode(point3d(0.0, 0.0, 0.0), true);
    float no_uncertainty = 0.0;
    uncertaintyNode->setUncertainty(no_uncertainty);
    uncertaintyTree.setNodeUncertainty(0.0, 0.0, 0.0, 0.0);
    uncertaintyTree.updateNode(point3d(0.1f, 0.1f, 0.1f), true);
    uncertaintyTree.setNodeUncertainty(0.1f, 0.1f, 0.1f, 0.5);

    EXPECT_TRUE(uncertaintyTree.write(filenameUncertainty));
    AbstractOcTree* readTreeAbstract = AbstractOcTree::read(filenameUncertainty);
    EXPECT_TRUE(readTreeAbstract);
    EXPECT_EQ(uncertaintyTree.getTreeType(),  readTreeAbstract->getTreeType());
    UncertaintyOcTree* readUncertaintyTree = dynamic_cast<UncertaintyOcTree*>(readTreeAbstract);
    EXPECT_TRUE(readUncertaintyTree);
    EXPECT_TRUE(uncertaintyTree == *readUncertaintyTree);
    uncertaintyNode = uncertaintyTree.search(0.0, 0.0, 0.0);
    EXPECT_TRUE(uncertaintyNode);
    EXPECT_EQ(uncertaintyNode->getUncertainty(), no_uncertainty);
    delete readUncertaintyTree;
  }
